...
class VisualiseObservation(PublisherLeaf):
    ...
    def __init__(self,
                 name=None,
                 topic_name='/observation/visualised',
                 topic_class=Image,
                 draw_bounding_box=True,
                 draw_overlay=True,
                 *args,
                 **kwargs):
        super(VisualiseObservation,
              self).__init__(name=name if name else 'Visualise observation',
    ...
    def load_fn(self):
        ...
        image = self.bridge.imgmsg_to_cv2(observation.rgb_image,
        ...
        for detection in observation.detections:
            ...
            if self.draw_bounding_box:
                image = cv2.rectangle(image,
                                      ...
                                      (detection.x_left + detection.width,
                                       detection.y_top + detection.height),
                                      color, 2)

                text_size, _ = cv2.getTextSize(detection.class_label,
                ...
                cv2.putText(image, detection.class_label,
                            (detection.x_left, detection.y_top - 4),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.3, 0, 1)

            if self.draw_overlay:
                roi = image[detection.y_top:detection.y_top + detection.height,
                            detection.x_left:detection.x_left +
                            detection.width, :]

                overlay = np.zeros((detection.height, detection.width, 3),
                                   np.uint8)
                overlay[:, :] = color

                overlay = cv2.bitwise_and(overlay,
                                          overlay,
                                          mask=self.bridge.imgmsg_to_cv2(
                                              detection.cropped_mask,
                                              desired_encoding='passthrough'))
    ...
    def colorize(self, word):
        ...
        first_num = ord(
            byte_digest[0])  # get int value of first character 0-255
        second_num = ord(
            byte_digest[1])  # get int value of second character 0-255
        ...
        return [
            int(color * 255)
            for color in colorsys.hsv_to_rgb(hue, saturation, brightness)
        ]
...
class VisualiseObservationCompressed(VisualiseObservation):
    ...
    def __init__(self,
                 name='Visualise observation',
                 topic_name='/observation/visualised/compressed',
                 *args,
                 **kwargs):
        super(VisualiseObservationCompressed,

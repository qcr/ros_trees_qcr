...
class GetMaskFromClick(Sequence):
  def __init__(self, shape=None, offset=None, expiry_time=None, *args, **kwargs):
      def load_fn(leaf):
          ...
          if observation.detections:
            ...
            obj.x_left=min(observation.depth_info.width - 1, max(0, x - _shape[0] / 2))
            obj.y_top=min(observation.depth_info.height - 1, max(0, y - _shape[1] / 2))

            obj.width = min(_shape[0]/2,x) + max(0, min(_shape[0]/2,observation.depth_info.width-x))
            obj.height = min(_shape[1]/2,y) + max(0, min(_shape[1]/2,observation.depth_info.height-y))

            obj.cropped_rgb = ros_numpy.msgify(Image, image[obj.y_top:int(obj.y_top+obj.height), obj.x_left:int(obj.x_left+obj.width)], encoding='rgb8')
            obj.cropped_depth = ros_numpy.msgify(Image, depth[obj.y_top:int(obj.y_top+obj.height), obj.x_left:int(obj.x_left+obj.width)], '32FC1')
            obj.cropped_mask = ros_numpy.msgify(Image, np.ones(shape=(obj.height, obj.width), dtype=np.uint8), encoding='8UC1')
      ...
      super(GetMaskFromClick, self).__init__(
       'Get mask from click',
       children=[
          GetClick(save_key='_mask_click', timeout=0.001, expiry_time=expiry_time if expiry_time else None),
          Leaf(
            name='Generate mask',
            load_fn=load_fn,
            save=True,

...
from ros_trees.leaves_common import TransformPose, TranslatePose
...
class MoveAboveTaggedLocation(Sequence):
    ...
    def __init__(self,
        ...
        super(MoveAboveTaggedLocation, self).__init__(children=[
            GetTaggedLocationPose(load_value=location if location else None,
                                  load_key=load_key if load_key else None,
                                  save_key='tag_pose'),
            TransformPose(target_frame='panda_link0',
                          load_key='tag_pose',
                          save_key='tag_pose'),
            TranslatePose(rx=math.pi,
                          rz=math.pi / 2,
                          load_key='tag_pose',
                          save_key='tag_pose'),
            TranslatePose(x=dx,

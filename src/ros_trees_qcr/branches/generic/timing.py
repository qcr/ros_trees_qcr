...
class TimedRelease(Sequence):
    ...
    def __init__(self, name='Timed Release', children=[], expiry_time=1):
        ...
        super(TimedRelease, self).__init__(
            ...
            children=[
                CheckTimer(
                    expiry_time=expiry_time, load_key=key, default_value=True)
            ] + children + [SetTimer(save_key=key)])


class SetTimer(Leaf):

    def __init__(self, name='Set Timer', *args, **kwargs):
        super(SetTimer, self).__init__(name=name,
...
class CheckTimer(Leaf):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(CheckTimer, self).__init__(name=name,
